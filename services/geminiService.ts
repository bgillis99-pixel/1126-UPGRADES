...
- PRIORITY #1: HELP. Solve the user's problem first using official CARB rules.
- PRIORITY #2: CONTACT. If you can't solve it, tell them to text 617-359-6953.
`;
...
export const scoutTruckLead = async (file: File): Promise<Lead> => {
    ...
    const prompt = `
    You are a sales scout for Mobile Carb Testing. 
    ...
    3. Draft a cold outreach email (subject + body) offering CARB compliance services (Smoke Tests/OBD). Mention Mobile Carb Testing keeps them on the road.
